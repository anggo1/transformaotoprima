                    ...
                    <div class="modal-content">
                        <div class="modal-header text-blue">
                            ...
                            <h5 style="display:block; text-align:center;"><span
                                    class="ion-soup-can-outline ion-lg text-blue"></span>&nbsp; Penerimaan Barang Order
                            </h5>
...
function showPart(id_part_order, no_part_order, kode_sup, supplier, status) {
    //var no_po = document.getElementById("no_po").value;
    //var no_po=document.getElementById("showPart");
    $.ajax({
        type: 'GET',
        url: '<?php echo base_url('Part_masuk_po/showPart'); ?>',
        data: 'id=' + id_part_order,
        success: function(hasil) {
            //MyTable.fnDestroy();//refresh();
            //$('#data_po').html(hasil);
            $('#data_po_detail').html(hasil);
            $('[name = "id_po"]').val(id_part_order);
            $('[name = "no_po"]').val(no_part_order);
...
function showPo1() {
    $.ajax({
        type: 'GET',
        url: '<?php echo base_url('Part_masuk_po/showPo'); ?>',
        //data: 'id_po=' + id_po,
        success: function(hasil) {
            MyTable.fnDestroy(); //refresh();
            //$('#data_po').html(hasil);
...
function showPo() {
    ...
    $.get('<?php echo base_url('Part_masuk_po/showPo'); ?>',
        function(data) {
...
function tampilDetail(dataPo) {
    ...
    $.ajax({
        type: 'GET',
        url: '<?php echo base_url('Part_masuk_po/tampilDetail'); ?>?id_po=' + id_po,
        data: 'id_po=' + id_po,
        success: function(hasil) {
            MyTable.fnDestroy();
...
$('#formpartmasuk').submit(function(e) {
    ...
    $.ajax({
            method: 'POST',
            url: '<?php echo base_url('Part_masuk_po/prosesPartmasuk'); ?>',
            data: data
        })
        .done(function(data) {
            var out = jQuery.parseJSON(data);
...
$(document).on("click", ".cetak-masuk", function() {
    ...
    $.ajax({
            method: "POST",
            url: "<?php echo base_url('Part_masuk_po/cetak'); ?>",
            data: "id=" + id
        })
        .done(function(data) {
