...
?> <tr>
        ...
        <td class="text-left"width=15%>
            ...
            <button type="button" class="btn btn-xs btn-outline-success generate"
                                        data-id="<?php echo $s->id_opname; ?>"  title="Generate"  data-toggle="modal" data-target="#generateOpname"><i
                                            class="fas fa-sync-alt"></i> Generate</button>
            <button type="button" class="btn btn-xs btn-outline-danger delete-opname" id="delete"
                                        data-id="<?php echo $s->id_opname; ?>"  title="Delete Data"  data-toggle="modal" data-target="#hapusOpname"><i
                                            class="fas fa-trash"></i> Delete</button>
                                            <?php } if($s->status=='Y'){  ?>
            <button class="btn btn-xs btn-outline-primary list-view" data-id="<?php echo $s->id_opname; ?>"><i class="fa fa-print"></i> Print</button>
            <?php } ?>
    </td>
    </tr>
<?php
...
var MyTable = $('#list-pk').DataTable({
    ...
    $(document).on("click", ".cetak-list", function() {
		...
		$.ajax({
				method: "POST",
				url: "<?php echo base_url('ReportOpname/cetakListOpname'); ?>",
				data: "id=" + id
			})
			.done(function(data) {
    ...
    $(document).on("click", ".list-view", function() {
		...
		$.ajax({
				method: "POST",
				url: "<?php echo base_url('ReportOpname/view_detailOpname'); ?>",
				data: "id=" + id
			})
			.done(function(data) {
				$('#modal-cetak-detail').html(data);
