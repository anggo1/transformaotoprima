                ...
                <form id="formPo" name="formPo" method="POST">
                    ...
                    <div class="modal-footer justify-content-between">
                        <label class="elegant-checkbox">
                            <input type="checkbox" name="service_only" id="service_only" value="Y" class="service_only" data-id="<?php echo $b->wo_no; ?>"
                                onclick="saveData(event,'<?php echo $b->wo_no; ?>')" />
...
<script type="text/javascript">
    ...
    const checkbox = document.getElementById('service_only');
    const button = document.getElementById('tambah-part');
    $('.service_only').on('change', function() {

        // 2. Only run the AJAX request if the checkbox is actively checked
        if (this.checked) {

            button.style.display = 'none';
            // Retrieve the wo_no data from the element (e.g., via a data attribute)
            var wo_no = $(this).data('id');

            $.ajax({
                type: "POST",
                url: "<?php echo base_url('EstimasiPenawaranService/update_serviceYes') ?>",
                data: {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url('EstimasiPenawaranService/update_serviceNo') ?>",
                data: {
                    '<?php echo $this->security->get_csrf_token_name(); ?>': '<?php echo $this->security->get_csrf_hash(); ?>',
                    'id': wo_no
                },
