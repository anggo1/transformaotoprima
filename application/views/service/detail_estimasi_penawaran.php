                        ...
                        ?>
                            <tr>
                                ...
                                <td title="Double click Untuk Edit Dan Tekan Enter untuk Simpan"
                                    onclick="this.contentEditable=true; this.className='inEdit';"
                                    onblur="this.contentEditable=false; this.className='inEdit';"
                                    onkeypress="saveHarga(event,'<?php echo $s->id_estimasi_penawaran; ?>','<?php echo $s->id_detail; ?>',$(this).html() )">
                                    <?php echo $s->harga_net; ?></td>
                                <td title="Double click Untuk Edit Dan Tekan Enter untuk Simpan"
                                    onclick="this.contentEditable=true; this.className='inEdit';"
                                    onblur="this.contentEditable=false; this.className='inEdit';"
...
<script language="javascript">
    ...
    function saveHarga(e, idpo, id, hrg_net) {
        if (e.keyCode === 13) {
            e.preventDefault();
            $.ajax({
                type: "POST",
                url: "<?php echo base_url('EstimasiPenawaranService/updateHarga') ?>",
                data: {
                    '<?php echo $this->security->get_csrf_token_name(); ?>': '<?php echo $this->security->get_csrf_hash(); ?>',
                    'id': id,
                    'hrg_part': hrg_net,
