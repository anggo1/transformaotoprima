...
class Mod_report_jobtime extends CI_Model
{
    ...
    function cari_jobtime($ttmp1 = null, $ttmp2 = null)
    {
        $this->db->select('a.wo_no', FALSE);
        $this->db->select('b.*', FALSE);
        $this->db->from('tbl_after_sales AS a');
        $this->db->join('tbl_after_sales_detail_wo AS b', 'b.wo_no=a.wo_no', 'left');
        $this->db->where('date_open_wo BETWEEN "' . $ttmp1 . '"AND"' . $ttmp2 . '"');
