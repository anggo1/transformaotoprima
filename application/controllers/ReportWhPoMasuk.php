...
class ReportWhPoMasuk extends MY_Controller
{

    public function __construct()
    {
        parent::__construct();
        $this->load->model(array('warehouse/Mod_reportwhpo_masuk', 'Mod_menu'));
    ...
    {
		$data['page'] 		= "Report Purchase Order Masuk";
		$data['judul'] 		= "List PO Masuk";
        $this->load->helper('url');
        $data['menu'] = $this->Mod_menu->getAll()->result();
        $this->template->load('layoutbackend', 'warehouse/report_wh/po_masuk', $data);
    }
    ...
    public function listPo() {
		...
		$data['dataPo'] = $this->Mod_reportwhpo_masuk->cari_po($ttmp1,$ttmp2); 
        $this->load->view('warehouse/report_wh/data_po_masuk', $data);
	}
	...
	{
		$id 				= $_POST['id'];
		$data['dataPo'] = $this->Mod_reportwhpo_masuk->select_by_id($id);
		$data['detailPo'] = $this->Mod_reportwhpo_masuk->select_detail($id);
    ...
    {
        $id = $_POST['id'];
        $result = $this->Mod_reportwhpo_masuk->deletePo($id);
