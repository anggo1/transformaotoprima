...
class Pegawai extends MY_Controller
{
        ...
        foreach ($viewLevel as $b) {
            ...
            foreach ($list as $p) {
                ...
                if ($b->edit_level == "Y" && $b->delete_level == "Y") {
                    $row[] = '
                <button class="btn btn-sm btn-outline-success update ion-compose ion-sm" onclick="edit_pegawai(\'' . $p->nip . '\')" title="Edit" data-id="' . $p->nip . '">
                </button>
                <button class="btn btn-sm btn-outline-danger delete ion-android-close ion-sm" title="Delete" onclick="delpegawai(\'' . $p->nip . '\')" data-toggle="modal" data-target="#hapusBody" data-id="' . $p->nip . '">
                </button>';
                }
                if ($b->edit_level == "Y" && $b->delete_level == "N") {
                    $row[] = '
                <button class="btn btn-sm btn-outline-success update ion-compose ion-lg" onclick="edit_pegawai(\'' . $p->nip . '\')" title="Edit" data-id="' . $p->nip . '">
                </button>';
                } else {
                    $row[] = '';
                }
    ...
    public function insert()
    {
        ...
        $this->Mod_pegawai->insertPegawai("tbl_hrd_pegawai", $save);
    ...
    public function delete()
    {
        ...
        $this->Mod_pegawai->deletepegawai($id, 'tbl_hrd_pegawai');
        $data['status'] = TRUE;
        echo json_encode($data);
    }
