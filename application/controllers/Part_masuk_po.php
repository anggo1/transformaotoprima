...
class Part_masuk_po extends MY_Controller
{

	function __construct()
	{
		parent::__construct();
		$this->load->model(array('warehouse/Mod_part_masuk_po'));
	...
	public function index()
	{
        ...
        $data['dataPo'] = $this->Mod_part_masuk_po->get_po();
        $data['dataSup'] = $this->Mod_part_masuk_po->get_sup();
        $data['dataKota'] = $this->Mod_part_masuk_po->get_kota();
		$this->template->load('layoutbackend', 'warehouse/part_masuk_po',$data);
	}

	public function cariKode($id)
	{
	$data = $this->Mod_part_masuk_po->get_part($id);
	echo json_encode($data);
	}
	public function showPart()
    {
		$id = $_GET['id'];
		//$data = $this->Mod_part_masuk_po->select_part($po);
		//$this->load->view('warehouse/data_po_partmasuk', $data);
		//if(!empty($po)){
        $data['dataPo'] = $this->Mod_part_masuk_po->select_part($id);
        $this->load->view('warehouse/detail_po_part', $data);
    }
	public function showNopo()
    {
		$tgl_po = $_GET['tgl_po'];
        $data = $this->Mod_part_masuk_po->select_nopo($tgl_po);
        //$this->load->view('warehouse/data_po_partmasuk', $data);
		echo json_encode($data);
    }
    ...
    {
		//$tgl_po = $_GET['tgl_po'];
        $data['dataPo']= $this->Mod_part_masuk_po->select_po();
        //$this->load->view('warehouse/data_po_partmasuk', $data);
	...
	{
		$id 				= $_GET['id_po'];
		$data['dataDetail'] = $this->Mod_part_masuk_po->select_detail($id);
		$this->load->view('warehouse/detail_part_masuk', $data);
	}
	public function deleteDetail()
	{
		$id = $_POST['id'];
		$result = $this->Mod_part_masuk_po->deleteDetail($id);
		if ($result > 0) {
	...
	public function updatePart()
	{
		...
		$data['dataEstimasi'] = $this->Mod_part_masuk_po->update_part($id,$qty_awal,$qty_masuk);
	...
	public function deletepartDetail()
	{
		$id = $_POST['id'];
		$sisa = $_POST['sisa'];
		$result = $this->Mod_part_masuk_po->deletepartDetail($id,$sisa);
		if ($result > 0) {
			//$out['datakode']=$kodeBaru;
			$out['status'] = '';
			$out['msg'] = show_del_msg('Deleted', '10px');
	...
	{
		$id 				= $_POST['id'];
		$data['dataMasuk'] = $this->Mod_part_masuk_po->select_by_id($id);
		$data['detailMasuk'] = $this->Mod_part_masuk_po->select_detail_cetak($id);

		echo show_my_print('warehouse/modals/modal_cetak_part_masuk_po', 'cetak-masuk', $data, ' modal-xl');
	}
}
