...
class ReportWhPartpk extends MY_Controller
{
    ...
    {
		$data['page'] 		= "Report Part";
		$data['judul'] 		= "PK/NonPK";
        $this->load->helper('url');
        $data['menu'] = $this->Mod_menu->getAll()->result();
        $this->template->load('layoutbackend', 'warehouse/report_wh/part_non', $data);
    }

    public function listPart() {
        $date1 				= $_GET['date1'];
        $date2 				= $_GET['date2'];
        $status_kategori 				= $_GET['kategori'];
        ...
        if($status_kategori=='pk'){
		$data['dataPart'] = $this->Mod_reportwhpo->cari_partpk($ttmp1,$ttmp2); 
        }
        if($status_kategori=='non'){
            $data['dataPart'] = $this->Mod_reportwhpo->cari_partnon($ttmp1,$ttmp2); 
            }
        $this->load->view('warehouse/report_wh/data_partnon', $data);
	}
    ...
    {
        $id = $_POST['id'];
        $result = $this->Mod_reportwhpo->deletePartnon($id);
